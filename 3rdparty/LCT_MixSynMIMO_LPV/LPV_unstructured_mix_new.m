...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ K,sol_info ] = LPV_unstructured_mix_new( sys,ny,nu,alpha,channel,param,varargin )
...
N = length(param);
opts.all_boundedness = zeros(N,1);
    for i = 1:N
        opts.all_boundedness(i) = strcmp(param{i}.bounded,'unbounded');
    end
    if(all(opts.all_boundedness) || (strcmp(opts.constantLyap,'true')))
        opts.controller_dependency = 'ada';
    end
switch sys.Ts
    case {0,{}}
        switch opts.controller_dependency
            case 'ada' % controller depending on parameter and its derivative
                ...
                assert(~isinf(sol_info.objective),'Sorry, SDP : INFEASIBLE problem, cannot go further, try relaxing constraints.');
            case 'a'   % controller depending on parameter only
                ...
                [K2,Primal2] = LPV_unstructured_mix_primal(sys,ny,nu,alpha,channel,param,opts,2);
                    ...
                    elseif Primal1.objective > Primal2.objective
                        ...
                        error('Sorry, SDP : INFEASIBLE problem, cannot go further, try relaxing constraints.')
                    end
            otherwise
                error('Wrong selection of controller dependency, must be ''a'' or ''ada''.');
        end
    otherwise 
                [K,sol_info] = LPV_unstructured_mix_primal_DT(sys,ny,nu,alpha,channel,param,opts);
end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
switch dep
    case 0 % X and Y dependent on constant and BRV parameters
        [X,dXdt] = build_Lyapmat_new(TB,param,nx,opti,'brv','c');
        [Y,dYdt] = build_Lyapmat_new(TB,param,nx,opti,'brv','c');
    case 1 % X depends on constant parameters only
        [X,dXdt] = build_Lyapmat_new(TB,param,nx,opti,'constant','c');
        [Y,dYdt] = build_Lyapmat_new(TB,param,nx,opti,'brv','c');
    case 2 % Y depends on constant parameters only
        [X,dXdt] = build_Lyapmat_new(TB,param,nx,opti,'brv','c');
        [Y,dYdt] = build_Lyapmat_new(TB,param,nx,opti,'constant','c');
    case 3
...
for i = 1:length(param)
    if ((strcmp(param{i}.bounded,'bounded')) && ~dep)
...
function [K,sol_info] = LPV_unstructured_mix_primal_DT(sys,ny,nu,alpha,channel,param,opts)
check_optispline;
import splines.*;
...
[X,dX] = build_Lyapmat(TB,param,nx,opti,'constant','d',deg,nknots);
[Y,dY] = build_Lyapmat(TB,param,nx,opti,'constant','d',deg,nknots);
...
sol_info.coeffs_Q    = Q.coeff.dimension;
sol_info.coeffs_dQ    = dQ.coeff.dimension;
for j = 1:n_pspecs
    sol_info.coeffs_Term{j} = Term{j}.coeff.dimension;
end
[pr,~]               = checkset(opti.yalmip_constraints); % primal residual
if pr > -opts.tolerance
    sol_info.feasible  = 1;
    sol_info.objective = sqrt(sol.value(objective));
    sol_info.gamma = sol.value(gamma).^2;
    switch opts.spec
        case {2,inf}
%             sol_info.gam2 = sol.value(gam2);
    end
...
sol_info.check_pos_Q = min(min_eig);
for j = 1:prod(sol_info.dQ.coeff.dimension)
    min_eig(j) = min(eig(sol_info.dQ.coeff.data(j))); 
end
sol_info.check_pos_dQ = min(min_eig);
for i = 1: n_pspecs
    for j = 1:prod(sol_info.Term{i}.coeff.dimension)
        max_eig(j) = max(eig(sol_info.Term{i}.coeff.data(j))); 
    end
