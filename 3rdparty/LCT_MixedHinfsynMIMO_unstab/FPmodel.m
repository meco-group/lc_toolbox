...
B = zeros(2*(n+1), 2);
B([ 1+2*actoffset, 2*(n-actoffset)+1 ], 1) = ones(2,1);
B(n+1, 2) = ones(1,1);
Cz = zeros([ 1, 2*(n+1) ]);
...
Ciz = zeros(1, 2*num_modes);
Ciy = zeros(1, 2*num_modes);

for m = 1:num_modes
    Ai([2*m-1, 2*m],[2*m-1, 2*m]) = [ 0, 1; -omegard(m)^2, -2*zeta(m)*omegard(m) ];
    Bi([2*m-1, 2*m],:) = [ zeros(1,2); Bm(m,:) ];
    Ciz(:,[2*m-1, 2*m]) = [ Cmz(:,m), zeros(1,1) ];
    Ciy(:,[2*m-1, 2*m]) = [ Cmy(:,m), zeros(1,1) ];
