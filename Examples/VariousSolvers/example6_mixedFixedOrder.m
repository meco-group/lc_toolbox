...
options = struct('FullOrderSolver','mixedHinfsynMIMO','gammasolver','lmilab','controller_name','mixed\_controller\_mimo');
[P,C1,info1] = P.solve(objective, constraints, K, options);
    
objective = [WS*S + Norm(WU*U,2)];
constraints = [MS*S <= 1];
options = struct('solver','mosek','controller_name','Mixed\_2norm');
[P,C2,info2] = P.solve(objective, constraints, K, options);

figure, bodemag(info1,info2,S,U,D);

% % Bode diagram of open loop
% figure, bode(std(G)*std(extract('Mixed\_Controller\_MIMO')),std(G)*std(extract('Mixed\_2norm')));
% % Bode diagram of the controllers
% figure, bode(std(extract('Mixed\_Controller\_MIMO')),std(extract('Mixed\_2norm')));
