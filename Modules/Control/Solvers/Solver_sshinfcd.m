...
classdef Solver_sshinfcd < Solver
    ...
    methods
        function self = Solver_sshinfcd(options)
            disp('solving with sshinfcd');
            
            self.solverobj = sshinfcd.sshinfcd(); 
        ...
        function self = solve(self,config,specs,vars)
            ...
            if isempty(specs.region)
                ...
                self.solverobj = self.solverobj.setproblem(P,Win,Wout,CH,reg);
            end
