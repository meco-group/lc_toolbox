...
classdef SystemOfSystems < AbstractSystem
    ...
    methods (Access=private)
        ...
        function self = resolve_lincomb(self)
            ...
            for k = 1:numel(unique_conn)
                if islincomb(unique_conn(k))
                    [mod,new_connection] = system(unique_conn(k),'autogenerated');
                    self = add(self,mod);
        ...
        function LFT = resolve_lft(self,varargin)
            ...
            function [blk,name,remove] = safeblkdiag(varargin)
                 try
                    blk = blkdiag(varargin{:});
                    names = cellfun(@(x) x.name,varargin,'un',0);
                    names(strcmp(names,'autogenerated')) = [];
                    name = strjoin(names,'-');
                    remove = false;
                 catch
            ...
            [cont,name,remove] = cellfun(@(x)safeblkdiag(x{:}),content__,'UniformOutput',false);
